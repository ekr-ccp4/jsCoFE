...
(if ``bzr`` is not installed on your system, obtain and install it from
http://bazaar.canonical.com). All jsCoFE servers: FE, NC and CS, use the same
source code, and the source code directory may be shared by any number and
types of servers, if permitted by the hardware setup. In the absence of shared
file system, the source code directory must be copied to every machine running
a jsCoFE server. Make sure that ``.bzr`` sub-directory is copied, too, for
future updates.
