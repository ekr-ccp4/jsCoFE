...
function CCP4Graph(graphName,d,xbreaks,ybreaks,options,shapes,titles,background,customXTicks,usePlotly){
...
function CCP4GraphPlot(graphDivName_in,showInput){
    ...
    var pushOut = document.createElement("div");
    pushOut.setAttribute("id","theButtonDiv");
    pushOut.setAttribute("style","float:right; position:relative; width:13%;");
    ...
    this.launchDataIDParam.setAttribute("value","");
    ...
    pushOut.appendChild(launchButton);
    ...
    menuContainer.appendChild(pushOut)
...
CCP4GraphPlot.prototype.loadXML = function(graphObject,root,tables) {
    ...
    for(var itab=0;itab<tables.length;itab++){
        ...
        for(var iplot=0;iplot<plots.length;iplot++){
            ...
            graph = new CCP4Graph(graphName,d,xbreak,ybreak,options,shapes,titles,background,customXTicks,usePlotly);
...
CCP4GraphPlot.prototype.plot = function() {
    ...
    if(typeof xbreak!=="undefined"&&xbreak.length>0){
        ...
        // FIXME - Ugly hardwire for Jon's plots. Should be more general. Ticks
        //         could be part of spec.
        if(yaxes.length>0 && yaxes[0]["min"] == 0 && yaxes[0]["max"] == 180){
            yaxes[0]["ticks"] = [0,60,120,180]
        }
        if(xaxes.length>0 && xaxes[0]["min"] == 0 && xaxes[0]["max"] == 360){
            xaxes[0]["ticks"] = [0,60,120,180,240,300,360]
        }
